...
- `tools/normalize_navigation_order.py`: 최종 navigation 정규화와 production origin을 `https://pc-checkup.pages.dev/`로 통일
...
`robots.txt`는 검색 crawler 접근을 허용하고 production sitemap을 제공합니다. 언어판 배포 완료, 검색엔진 등록 완료, 실제 색인·검색유입 발생은 서로 다른 상태로 관리합니다.

Cloudflare Pages를 현재 production origin으로 사용합니다. 중국 본토의 안정적 접근성을 보장하는 별도 도메인/호스팅과 Baidu 사이트 소유확인·제출은 아직 외부 Gate입니다. Yandex Webmaster 정식 연결도 자체도메인 및 소유확인 Gate가 남아 있습니다.
...
- Cloudflare Pages 최초 수동 배포 SUCCESS 확인
