...
for (const item of representative) {
  ...
  assert((await page.locator('html').getAttribute('lang')) === item.lang, `${item.path}: html lang mismatch`);
...
console.log(`DEVICE CHECKUP production QA passed: ${urls.length} sitemap URLs reachable; representative ko/en/ja/zh-CN/ru mobile surfaces and tool pages had no page errors, no horizontal overflow on home surfaces, and no unexpected external network origins.`);
